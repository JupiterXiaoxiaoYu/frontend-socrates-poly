      ...
      <DialogContent className="h-full w-full max-w-full md:max-w-md md:h-auto md:max-h-[calc(100vh-120px)] left-0 top-0 md:left-[50%] md:top-[50%] translate-x-0 translate-y-0 md:translate-x-[-50%] md:translate-y-[-50%] overflow-y-auto rounded-none md:rounded-lg p-0 gap-0 border-0 md:border">
        ...
        <div className="sticky top-0 z-10 bg-background border-b border-border md:hidden">
          <div className="flex items-center h-14 px-4">
            ...
            <h2 className="flex-1 text-center text-base font-bold text-foreground pr-8">{t('dashboard')}</h2>
          </div>
        </div>

        {/* 桌面端标题 */}
        <DialogHeader className="hidden md:flex p-6 pb-4">
          <DialogTitle className="text-base font-bold">{t('dashboard')}</DialogTitle>
        </DialogHeader>

        <div className="space-y-4 p-4 md:px-6 md:pb-6 md:pt-0">
          {/* Referrals */}
          <Card className="p-4 border border-border bg-card">
            <h3 className="text-base font-bold text-foreground mb-4">{t('referrals')}</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">{t('directReferrals')}</span>
                <span className="font-bold text-foreground">22</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">{t('indirectReferrals')}</span>
          ...
          {/* Trading Volume */}
          <Card className="p-4 border border-border bg-card">
            <h3 className="text-base font-bold text-foreground mb-4">{t('tradingVolume')}</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">{t('directReferrals')}</span>
          ...
          {/* Fee Rebate */}
          <Card className="p-4 border border-border bg-card">
            <h3 className="text-base font-bold text-foreground mb-4">{t('feeRebate')}</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">{t('directReferrals')}</span>
                <span className="font-bold text-foreground">22 USDC</span>
          ...
          {/* Mining Rebate */}
          <Card className="p-4 border border-border bg-card">
            <h3 className="text-base font-bold text-foreground mb-4">{t('miningRebate')}</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">{t('directReferrals')}</span>
