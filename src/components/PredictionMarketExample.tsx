        ...
        <div className="max-w-6xl mx-auto p-6 space-y-8">
            ...
            <div className="bg-white rounded-lg shadow p-6">
                <h1 className="text-3xl font-bold text-gray-900 mb-4">
                    zkWASM Prediction Market Example
                </h1>
                ...
                {transactionState.status !== 'IDLE' && (
                    <div className={`mt-4 p-3 rounded ${
                        transactionState.status === 'SUCCESS' ? 'bg-green-100 text-green-800' :
                        transactionState.status === 'ERROR' ? 'bg-red-100 text-red-800' :
                        'bg-blue-100 text-blue-800'
                    }`}>
                        <div className="font-medium">
                            {transactionState.type && (
                                <span className="mr-2">{transactionState.type.replace('_', ' ')}</span>
                            )}
                            {transactionState.status}
                        </div>
                        {transactionState.error && (
                            <div className="text-sm mt-1">{transactionState.error}</div>
                        )}
                    </div>
                )}
                                ...
                                <div key={market.marketId} className="border rounded-lg p-4 hover:bg-gray-50">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="text-right">
                                            ...
                                            <div className={`text-xs mt-1 px-2 py-1 rounded ${
                                                ...
                                                market.status === 'RESOLVED' ? 'bg-purple-100 text-purple-800' :
                                    ...
                                    <div className="mt-3 flex space-x-2">
                                        ...
                                        {market.status === 'RESOLVED' && positions.some(p => p.marketId === market.marketId) && (
                                            <button
                                                onClick={() => claimWinnings({ marketId: BigInt(market.marketId), outcome: market.resolvedOutcome! })}
