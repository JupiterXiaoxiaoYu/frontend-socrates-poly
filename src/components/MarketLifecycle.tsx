...
import { cn, formatDuration, formatTimeRemaining, formatCurrency, formatPercent } from '@/lib/utils';
...
const MarketLifecycle: React.FC<MarketLifecycleProps> = ({
  ...
  const [globalState, setGlobalState] = useState({ counter: 0 });
  ...
  useEffect(() => {
    ...
    const unsubscribeGlobal = webSocketService.onGlobalUpdate((update) => {
      setGlobalState(update);
    });

    const unsubscribeMarket = webSocketService.onMarketUpdate(market.marketId, (update) => {
  ...
  const getLifecycleEvents = (): LifecycleEvent[] => {
    ...
    events.push({
      ...
      description: `Prediction market for ${market.assetId === 1 ? 'BTC' : market.assetId === 2 ? 'ETH' : 'SOL'} ${market.outcomeType === OutcomeType.UP ? 'UP' : 'DOWN'}`,
    ...
    events.push({
      ...
      status: currentTick >= closeTick ? 'completed' :
              currentTick >= startTick && currentTick < closeTick ? 'current' : 'pending'
    });
  ...
  const getStatusColor = (status: MarketStatus): string => {
    switch (status) {
      case MarketStatus.ACTIVE: return 'text-green-600 bg-green-50 border-green-200';
      case MarketStatus.PENDING: return 'text-yellow-600 bg-yellow-50 border-yellow-200';
      case MarketStatus.CLOSED: return 'text-orange-600 bg-orange-50 border-orange-200';
      case MarketStatus.RESOLVED: return 'text-blue-600 bg-blue-50 border-blue-200';
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              ...
              <div className="text-lg font-bold">
                {formatCurrency(market.volume || 0, 0)}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {market.oracleStartPrice && (
                  <div>
                    ...
                    <div className="font-semibold">
                      {formatCurrency(market.oracleStartPrice)}
                    </div>
                    <div className="text-xs text-muted-foreground">
                ...
                {market.oracleEndPrice && (
                  <div>
                    ...
                    <div className="font-semibold">
                      {formatCurrency(market.oracleEndPrice)}
                    </div>
                    <div className="text-xs text-muted-foreground">
              ...
              {market.oracleStartPrice && market.oracleEndPrice && (
                <div className="mt-2">
                  ...
                  <div className={cn(
                    ...
                    {market.priceChange! > 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" />}
                    {formatCurrency(market.priceChange!)} ({formatPercent(market.priceChangePercent! / 100, 2)})
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="border-t pt-4">
              ...
              <div className={cn(
                ...
                <div className="text-lg font-bold mb-1">
                  {market.winningOutcome === (market.outcomeType === OutcomeType.UP ? Outcome.UP : Outcome.DOWN) ? '✓ YOU WON' : '✗ YOU LOST'}
                </div>
                <div className="text-sm">
                  Winning outcome: {getWinningOutcomeText()}
                </div>
              </div>
            </div>
          )}
          ...
          {pairedMarket && showDetails && (
            <div className="border-t pt-4">
              ...
              <div className="text-sm space-y-1">
                ...
                <div>
                  <span className="text-muted-foreground">Type:</span> {pairedMarket.outcomeType === OutcomeType.UP ? 'UP' : 'DOWN'}
                </div>
                <div>
              ...
              <div className="flex flex-wrap gap-2">
                {market.status === MarketStatus.ACTIVE && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => onMarketAction('close', market.marketId)}
                  >
                ...
                {market.status === MarketStatus.CLOSED && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => onMarketAction('resolve', market.marketId)}
                  >
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => onMarketAction('details', market.marketId)}
                >
                  View Details
                </Button>
              </div>
