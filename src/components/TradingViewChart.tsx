...
export const TradingViewChart: React.FC<TradingViewChartProps> = ({
  ...
  const convertToChartData = useCallback((priceHistory: any[]): ChartDataPoint[] => {
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      grid: {
        vertLines: { color: 'rgba(197, 203, 209, 0.3)' },
        horzLines: { color: 'rgba(197, 203, 209, 0.3)' },
      },
      crosshair: {
  ...
  }, [priceHistory, currentPrice, convertToChartData, showVolume]);

  // 实时价格更新
  useEffect(() => {
    if (!autoUpdate || !candlestickSeriesRef.current || !currentPrice) return;
    ...
    if (lastDataPoint && Math.abs(Number(lastDataPoint.time) - time) < 60) {
      // 更新当前时间段的数据
      const updatedPoint = {
        ...lastDataPoint,
        close: price,
    ...
    setLastUpdate(new Date());
  }, [currentPrice, autoUpdate, chartData]);
    ...
    if (chartRef.current && chartContainerRef.current) {
      setTimeout(() => {
        chartRef.current?.applyOptions({
          width: chartContainerRef.current?.clientWidth,
          height: isFullscreen ? height : window.innerHeight - 100,
        });
      }, 100);
    }
  };
